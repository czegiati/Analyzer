        ...
        if(o.getClass().isAnnotationPresent(AttributeDependent.class)) {
            ...
            for(Method m:class0.getMethods()){
                if(m.isAnnotationPresent(Interceptor.class)) {
                    counter++;
                    if (counter > 1) throw new IllegalArgumentException("There can only be one Method with the Interceptor annotation!");
                    ...
                    for(Parameter p: m.getParameters()){
                        ...
                        {
                            if(attriblites.get(p.getAnnotation(AttributeParam.class).value())==null && p.getAnnotation(AttributeParam.class).required()) throw new IllegalArgumentException("Attribute "+p.getAnnotation(AttributeParam.class).value()+" cannot be found, while it is required for method: "+m.getName()+" in class: "+o.getClass());
        ...
        for(Class c0:restrictions){
            if(o.getClass().isAnnotationPresent(c0)){ //a restriction is placed on this class
                ...
                if(annotation.annotationType().isAnnotationPresent(RestrictionAnnotation.class)) {
                    ...
                    } else throw new IllegalArgumentException("You have to implement the RestrictionHandler interface int your restriction's body :"+interf);
                }
                else throw new IllegalArgumentException("You cannot register an annotation as Restriction without annotating it with @RestrictionAnnotation: "+annotation.getClass());
            }
        }
    }
